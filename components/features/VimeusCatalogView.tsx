...
interface VimeusCatalogViewProps {
  endpoint: "movies" | "series" | "animes";
  title: string;
  subtitle: string;
}
...
export function VimeusCatalogView({ endpoint, title, subtitle }: VimeusCatalogViewProps) {
  ...
  const hasFilters = Boolean(filters.q || filters.genre || filters.year || filters.minRating);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await fetch(`/api/vimeus/${endpoint}?${params}`, {
